...
public class Field650Converter extends Field648Converter {
    ...
    protected Model process(VariableField field) {
        ...
        Resource madsClass =  df.getSubfields("bcdvxyz").isEmpty() ?
        ...
        if (MADS_RDF.ComplexSubject.equals(madsClass)) {
            addComponentList(df, lang, resource, "abcdvxyz");
        }
    ...
    public boolean checkField(VariableField field) {
        return "650".equals(getTag(field));
    }
    ...
    protected Resource getComponentType(char code) {
        switch (code) {
            case 'a':
            case 'b': return MADS_RDF.Topic;
            case 'c': return MADS_RDF.Geographic;
            case 'd': return MADS_RDF.Temporal;
    ...
    protected Property getComponentProperty() {
        return MADS_RDF.authoritativeLabel;
    }
}
