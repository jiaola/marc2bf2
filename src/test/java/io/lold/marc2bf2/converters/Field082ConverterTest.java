...
public class Field082ConverterTest {
    Field082Converter converter;
    private Model model;
    ...
    public void setUp() throws Exception {
        ...
        converter = new Field082Converter(model, record);
    }
    ...
    public void testConvert() throws Exception {
        String q = String.join("\n"
                ...
                , "SELECT ?x  "
                , "WHERE { "
                , "  ?x rdf:type bf:ClassificationDdc ."
                , "  ?x bf:source ?s ."
                , "  ?s rdfs:label \"DE-101b\" ."
                , "  ?x bf:edition \"full\" ."
                , "}");
        List<DataField> fields = record.getDataFields();
        for (DataField field: fields) {
            if (field.getTag().equals("082") && field.getIndicator2() == '4') {
