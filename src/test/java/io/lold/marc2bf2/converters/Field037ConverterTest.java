...
public class Field037ConverterTest {
    Field037Converter converter;
    private Model model;
    ...
    public void setUp() {
        ...
        converter = new Field037Converter(model, record);
    }
    ...
    public void testConvert1() throws Exception {
        String q = String.join("\n"
                ...
                , "SELECT ?w  "
                , "WHERE { "
                , "  ?w bf:acquisitionSource ?a ."
                , "  ?a rdf:type bf:AcquisitionSource ."
                , "  ?a bf:identifiedBy ?i ."
                , "  ?i rdf:value \"FSWEC-77/0420\" ."
                , "  ?a bf:note ?n ."
                , "  ?n rdfs:label \"current source\" ."
                , "}");
        List<DataField> fields = record.getDataFields();
        for (DataField field: fields) {
            if (field.getTag().equals("037") && field.getIndicator1() == '3') {
    ...
    public void testConvert2() throws Exception {
        String q = String.join("\n"
                ...
                , "  ?a rdf:type bf:AcquisitionSource ."
                , "  ?a bf:identifiedBy ?i ."
                , "  ?i rdf:value \"FOO_FSWEC-77/0420\" ."
                , "}");
        List<DataField> fields = record.getDataFields();
        for (DataField field: fields) {
            if (field.getTag().equals("037") && field.getIndicator1() == ' ') {
                model = converter.convert(field);
                model.write(System.out);
                ResultSet rs = TestUtils.sparql(q, model);
                assertTrue(rs.hasNext());
